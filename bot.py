...
async def send_reminder(chat_id: int, stage: dict, target_block: int, kind: str) -> bool:
    ...
    text = (
        f"{prefix}{stage['emoji']} <b>{stage['title']}</b>\n\n"
        f"{stage['teaser']}\n\n"
        "Открой приложение — продолжим отсюда 👇"
    )
    try:
...
async def run_reminders() -> int:
    """Один проход рассылки по ЭТАПАМ. Возвращает число отправленных напоминаний."""
    ...
    for u in users:
        ...
        elif (day_num >= target["day"] + FOLLOWUP_GAP_DAYS
              and follow_key not in sent_keys):
            key, kind = follow_key, "follow"
        else:
            continue

        if await send_reminder(chat_id, target, target_block, kind):
