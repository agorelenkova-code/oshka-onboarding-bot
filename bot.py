...
LESSON_ORDER = [1, 2, 4, 21, 3, 18, 5, 8, 9, 13, 14, 15, 16, 19, 20]
# Порядок и дни должны совпадать с курсом (index.html VISIBLE / tg-onboarding.js ORDER)
LESSON_DAY = {1: 1, 2: 2, 4: 3, 21: 4, 3: 5, 18: 6, 5: 7, 8: 10,
              9: 22, 13: 35, 14: 38, 15: 42, 16: 45, 19: 46, 20: 48}
LESSON_TITLE = {
    1: "Знакомство со школой", 2: "Входим на платформу", 4: "Познакомься с куратором",
    21: "Личный кабинет", 3: "Покоряем Zoom", 18: "Как проходит обучение",
    5: "Первые уроки", 8: "ДЗ, тесты, самопроверка", 9: "Загружаем документы",
    13: "Порядок аттестации", 14: "Правила и критерии", 15: "Доклады и рефераты",
    16: "Стратегия аттестации", 19: "ИС + ЕГЭ/ОГЭ", 20: "Дополнительные занятия",
}
# Превью-тизер для напоминаний: эмодзи + короткое описание (нейтрально по полу)
LESSON_PREVIEW = {
    1: ("🏫", "С этого начинается учёба в школе. Короткий вводный шаг — познакомимся и настроимся на старт."),
    2: ("🖥", "Покажем, как зайти на платформу: установить браузер, найти письмо, создать пароль и войти."),
    4: ("🧑‍🏫", "Куратор — главный помощник в школе. Расскажем, кто это и как быть с ним на связи."),
    21: ("🖥", "Гид по личному кабинету: вкладки ученика и родителя, дневник, расписание, поддержка."),
    3: ("🎥", "Все уроки проходят в Zoom. Научимся устанавливать его, входить и подключаться к занятию."),
    18: ("📚", "Как устроено обучение: живые уроки и записи, оценки, форматы занятий."),
    5: ("🎉", "Как проходят первые занятия и где смотреть расписание."),
    8: ("📝", "Домашние задания, тесты и самопроверка: виды, где найти и как загрузить работу."),
    9: ("📂", "Какие документы нужны для зачисления и как правильно их загрузить."),
    13: ("📋", "Аттестация — важный этап. Разберёмся, как она проходит и где найти работы, чтобы подготовиться заранее."),
    14: ("📖", "Как оценивают работы и где официальные правила аттестации."),
    15: ("📝", "Как оформлять и сдавать доклады и рефераты."),
    16: ("🏆", "Как спланировать аттестацию и пройти её спокойно."),
    19: ("🎓", "Подготовка к ГИА: помощь с ОГЭ и ЕГЭ, допуски и пробные экзамены."),
    20: ("🎨", "Дополнительные занятия: бесплатные кружки, вебинары, факультативы и продлёнка."),
}
...
async def send_reminder(chat_id: int, lesson: int, kind: str) -> bool:
    ...
    emoji, teaser = LESSON_PREVIEW.get(lesson, ("📌", ""))
    kb = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="▶️ Открыть урок", web_app=WebAppInfo(url=url))
    ]])
    prefix = "" if kind == "due" else "🙌 Ещё раз напоминаем\n\n"
    text = (
        f"{prefix}{emoji} <b>{title}</b>\n\n"
        f"{teaser}\n\n"
        "Открой приложение — там этот шаг целиком 👇"
    )
    try:
