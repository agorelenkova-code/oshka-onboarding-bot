...
# --- SMTP для писем веб-пользователям (у кого нет Telegram) -----------------
# Если не заданы — письма просто не шлём (Telegram-напоминания работают как обычно).
SMTP_HOST = os.environ.get("SMTP_HOST", "")
SMTP_PORT = int(os.environ.get("SMTP_PORT", "465"))
SMTP_USER = os.environ.get("SMTP_USER", "")
SMTP_PASS = os.environ.get("SMTP_PASS", "")
SMTP_FROM = os.environ.get("SMTP_FROM", "") or SMTP_USER
SMTP_FROM_NAME = os.environ.get("SMTP_FROM_NAME", "Онлайн-школа №1")
...
STAGES = [
    {"key": "S1", "day": 1, "emoji": "👋", "title": "Знакомство и первый вход",
     "teaser": "С этого начинается учёба: знакомимся со школой, входим на платформу, встречаем куратора и осваиваем личный кабинет.",
     "blocks": [1, 2, 4, 21]},
    {"key": "S2", "day": 2, "emoji": "🎬", "title": "Первые уроки",
     "teaser": "Готовимся к занятиям: Zoom, как устроено обучение, первые уроки и домашние задания.",
     "blocks": [3, 18, 5, 8]},
...
def _course_link(target_block: int, u: dict, email: str) -> str:
...
def _send_email_sync(to: str, subject: str, html: str, text: str) -> None:
    msg = EmailMessage()
...
async def send_email_reminder(email: str, stage: dict, target_block: int, u: dict) -> bool:
    """То же дружеское напоминание, но письмом (для веб-пользователей без Telegram)."""
    if not (SMTP_HOST and SMTP_USER and SMTP_PASS and email and "@" in email):
        if not SMTP_HOST:
            log.warning("SMTP не настроен — письмо %s не отправлено", email)
        return False
    link = _course_link(target_block, u, email)
    subject = f"Напоминание: {stage['title']} — Онлайн-школа №1"
    text = (
        f"Дружеское напоминание.\n\n{stage['title']}\n{stage['teaser']}\n\n"
        f"Продолжить курс: {link}\n"
    )
    html = f"""\
<div style="font-family:-apple-system,Segoe UI,Roboto,sans-serif;max-width:520px;margin:0 auto;color:#241539">
  <p style="font-size:13px;color:#7c3aed;font-weight:700;margin:0 0 6px">👋 Дружеское напоминание</p>
  <h2 style="font-size:20px;margin:0 0 10px">{stage['emoji']} {stage['title']}</h2>
  <p style="font-size:15px;line-height:1.5;color:#4b3a63;margin:0 0 20px">{stage['teaser']}</p>
  <a href="{link}" style="display:inline-block;background:#7c3aed;color:#fff;text-decoration:none;
     font-weight:700;font-size:15px;padding:12px 22px;border-radius:12px">Продолжить курс →</a>
  <p style="font-size:12px;color:#8a7aa6;margin:22px 0 0">Онлайн-школа №1 · онбординг</p>
</div>"""
    try:
        await asyncio.to_thread(_send_email_sync, email, subject, html, text)
        return True
    except Exception as e:
        log.warning("Письмо %s не отправлено: %s", email, e)
        return False


async def run_reminders() -> int:
    """Один проход рассылки по ЭТАПАМ (Telegram + email). Возвращает число отправленных."""
    ...
    for u in users:
        ...
        if uid.startswith("web:"):
            ...
            ok = await send_email_reminder(email, target, target_block, u)
        else:
            try:
                chat_id = int(uid)
            except ValueError:
                continue
            ok = await send_reminder(chat_id, target, target_block)
